...
function pre_customize_image__add_tailscale_to_image() {
	...
	if [[ "${DISTRIBUTION}" == "Debian" ]]; then
		...
		run_host_command_logged echo "deb [arch=${ARCH} signed-by=/usr/share/keyrings/tailscale.gpg] https://pkgs.tailscale.com/stable/debian ${RELEASE} main" "|" tee "${SDCARD}"/etc/apt/sources.list.d/tailscale.list
	elif [[ "${DISTRIBUTION}" == "Ubuntu" ]]; then
		display_alert "Adding sources.list for Tailscale Inc." "Ubuntu :: ${EXTENSION}" "info"
		run_host_command_logged echo "deb [arch=${ARCH} signed-by=/usr/share/keyrings/tailscale.gpg] https://pkgs.tailscale.com/stable/ubuntu ${RELEASE} main" "|" tee "${SDCARD}"/etc/apt/sources.list.d/tailscale.list
	else
